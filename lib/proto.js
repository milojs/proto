 ...
 */
var	prototypeFunctions = require('./proto_prototype');
...
// objectMethods.extend(Proto, objectMethods, true);
objectMethods.extend.call(Proto, prototypeFunctions, true);
// _.extend(Proto, arrayMethods, true);
// _.extend(Proto, stringMethods, true);
// _.extend(Proto, functionMethods, true);
...
Object.keys(Proto).forEach(function(funcName) {
	protoInstanceMethods[funcName] = utils.makeProtoInstanceMethodFromFunc(Proto[funcName]);
});
...
objectMethods.extend.call(Proto, arrayMethods.__nativeFunctions, true);

// allow chaining for native array methods
var protoInstanceArrayMethods =
		objectMethods.mapKeys.call(
...
function addFuncsAndMethodsToProto(methodsMap) {
	objectMethods.eachKey.call(methodsMap, function(method, methodName) {
		Proto[methodName] = utils.makeProtoFunction(method);
		objectMethods.defineProperty.call(Proto.prototype, methodName,
