 ...
 * @param {List} args Arguments after self will be prepended to the original function call when the partial function is called.
 * @return {Function}
 */
function partial(...args) {
    var func = this;
    return function(...callArgs) {
        return func.apply(this, args.concat(callArgs));
    };
}
 ...
 *
 * @param {Function} self Function to be applied
 * @param {List} args Arguments after self will be appended on the right to the original function call when the partial function is called.
 * @return {Function}
 */
function partialRight(...args) { // , ... arguments
    var func = this;
    return function(...callArgs) {
        return func.apply(this, callArgs.concat(args));
    };
}
 ...
 * @param {List} args optional arguments that will be passed to the function
 */
 ...
 * @param {List} args optional arguments that will be passed to the function
 */
function deferTicks(ticks, ...args) {
    if (ticks < 2) return defer.apply(this, args);
    var deferArgs = repeat.call(deferFunc, ticks - 1);
    deferArgs = deferArgs.concat(this, args); 
    return deferFunc.apply(null, deferArgs);
}
 ...
 * @param {Number} wait approximate dalay time in milliseconds
 * @param {List} args arguments to pass to method
 */
function delayMethod(funcOrMethodName, wait, ...args) {
 ...
 * @param {List} args arguments to pass to method
 */
function deferMethod(funcOrMethodName, ...args) {
 ...
 * @return {Function}
 */
function delayed(wait, ...args) {
    var func = this;
    return function(...callArgs) {
        var passArgs = args.concat(callArgs);
