...
var objectFuncs = require('./dotjs/functions/object');
var objectMethods = require('./dotjs/methods/object');

for (var name in objectFuncs) {
    Proto[name] = objectFuncs[name];
    Proto.prototype[name] = objectMethods[name];
}
...
for (var name in objectFuncs._constants) {
    Proto[name] = objectFuncs._constants[name];
}
